...
class AnnotationTool:
    ...
    def load_image(self):
        if 0 <= self.current_image_index < len(self.image_list):
            ...
            self.canvas.create__image(0, 0, anchor=tk.NW, image=self.tk_image)
        ...
        with open(os.path.join(plm_data_dir, "train.jsonl"), "w") as f:
            for i, image_name in enumerate(self.image_list):
                ...
                if os.path.exists(annotation_path):
                    ...
                    # A simple copy, in real life, you might want to handle file existing cases
                    import shutil
                    shutil.copy(image_path, new_image_path)

                    # Create conversation template
                    # For object detection, we can format the labels and boxes
                    assistant_response = ""
    ...
    def start_training(self):
        ...
        config = {
            ...
            "checkpoint": {
                "init_ckpt_path": os.path.join("models", model_name)
            }
        }
        with open(config_path, 'w') as f:
            yaml.dump(config, f)

        # Add dataset config to the main datasets.yaml
        # This is a simplification. In a real scenario, you would want to manage this more robustly.
        with open(os.path.join(perception_models_path, "apps/plm/configs/datasets.yaml"), "a") as f:
            f.write("\n")
            f.write("plm_finetune:\n")
            f.write(f"  annotation: {os.path.join(plm_data_dir, 'train.jsonl')}\n")
            f.write(f"  root_dir: {os.path.join(plm_data_dir, 'images')}\n")

        self.log(f"Starting training with model: {model_name}")
        self.train_button.config(state=tk.DISABLED)
        self.progress.start()
        
        train_thread = threading.Thread(target=self.training_thread, args=(perception_models_path, config_path))
        train_thread.start()

    def training_thread(self, perception_models_path, config_path):
        try:
            command = [
                "torchrun", "--nproc-per-node", "1", # Simplified for single GPU
                "-m", "apps.plm.train",
